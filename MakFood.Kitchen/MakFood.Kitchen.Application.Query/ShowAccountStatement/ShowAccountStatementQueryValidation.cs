    ...
    public class ShowAccountStatementQueryValidation : AbstractValidator<ShowAccountStatementQuery>
    {
        public ShowAccountStatementQueryValidation()
        {
            RuleFor(x => x.CustomerId.ToString()).NotEmpty()
                .WithMessage($"The CustomerId is Not Empty");

            RuleFor(x => x.StartDateTime).NotEmpty()
                .WithMessage("The DateTime is Not Empty")
                .When(x => x.StartDateTime <= DateTime.Now && x.StartDateTime <= x.EndDateTime)
                .WithMessage("The entered Date is not valid.");

            RuleFor(x => x.EndDateTime).NotEmpty()
                .WithMessage("The DateTime is Not Empty")
              .When(x => x.EndDateTime <= DateTime.Now && x.EndDateTime >= x.StartDateTime)
              .WithMessage("The entered Date is not valid.");
        }
    }
}
